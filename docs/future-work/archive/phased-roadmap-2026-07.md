# The phased roadmap, as it stood 2026-07 (history)

Phases 1â€“4 as originally sequenced, with the completion notes written as each landed. **All four are
...
This is not a cosmetic agent swap. It advances kernel modularity, context efficiency, makerâ‰ checker
critics, drift-resistant stop conditions, and empirical tuning (`heuristics-tuner` as the meta-loop).

### Phase 1 â€” The general MCP agent âœ… (done, 2026-07-02)

The vault-agnostic interactive agent; proves the core runs without TurboVault (none of it touches the
vault).

- âœ… **Route chat through the dispatcher.** Chat now runs every turn through `Dispatcher::dispatch`
...
- âœ… **Live capability catalog + on-demand tool surfacing.** The three independently-static catalog
  ...
  configurable via `DispatchTuning::narrow_direct_tools`, default on) â€” verified live: a PDF goal
...
- âœ… **Multi-MCP + parallel, capability-narrowed sub-delegation** (component-scoping half).
  `Grant.component` is now consulted (`"main-agent"` vs `"dispatcher"`), and the `ExecuteDirect`
  runtime-scoping gap that made this toothless is closed â€” see the dispatcher-and-component-scoping
  ...
  deferred MCPs â€” caldav, calorie-counter, weather pending an upstream stdio fix â€” are usable too).

### Phase 2 â€” The self-improvement moat âœ… (done, June 2026)

Self-extension is, at bottom, **the agent using a code-building MCP** â€” no new dispatch-action,
...
(Decision 14 â€” the daemon never writes config) + a restart. Hermes gap #1, containably. Full plan:
...
("mesh checkpoint #2") are **deferred** â€” riskiest, lowest value now, and runtime config-writes would
break Decision 14; a restart activates the merged MCP.

**Riggers â€” the self-improvement engine, complete.** `riggers/` (`liberado-pr-dispatch-mcp`) is
...
1. âœ… **Register it as `code-dispatch` (`reversible`), grant `ExecuteMcp("code-dispatch")`, and switch it
...
2. âœ… **Add a greenfield mode** â€” the one genuinely new capability. Greenfield scaffolds a fresh MCP
   ...
   absent -> `create`.
3. âœ… **Eval + docs + dogfooding migration** â€” 6 eval scenarios verify the single-gate and
...
### Before Phase 3 â€” heuristics tuning engine (dispatcher, executor, and subagent layers built â€” 2026-07-06)

Automate the manual "run eval â†’ read the misses â†’ tune the prompt â†’ run again" loop
`liberado-eval` already documents doing by hand, so weak points in routing/tool-use surface
proactively instead of through slow dogfooding â€” the goal is a solid tool-use architecture
*before* Phase 3 autonomy breadth widens the surface area further. New crate
`liberado-heuristics-tuner` (using `provider-openai-compat`'s `openrouter_from_env()` â€” many
...
- **âœ… Multi-step tool chaining reliability â€” substantially resolved (2026-07-04).** Found via the
  tuner, not a tuner-specific problem â€” both `ExecuteDirect` and `DispatchSubagent` share the same
  ...
  match) that escalates nudge â†’ tool removal (with a one-time bounded turn-budget top-up) â†’ honest
  failure, plus a progress-aware budget-exhaustion report. Live-verified 0/6 â†’ 5/6 on the original
...
- **âœ… Resource-budget hardening (2026-07-04).** Ahead of Phase 3 widening the autonomy surface â€”
  ...
  for cost (real `$`/token pricing deferred â€” not worth the upkeep while usage is this cheap) are
  wired in now; every existing `Budget::new`/`Budget::default` call site is unchanged.
- **âœ… Zone-write-class guard (Â§6 #2) â€” done (2026-07-04).** See the "Landed" section below for the
...
- **âœ… Proposal notifications (Telegram) â€” done (2026-07-04).** Closes the last "who's watching an
  ...
  `Notifier` trait (Telegram is the first implementation â€” free, mature, works today; a future
  ...
  constructor call site needed to change. Always best-effort â€” a failed notification never blocks
...
- **âœ… Two-way Telegram approval (approve/reject/revise) â€” done (2026-07-04).** Closes the follow-up
  question the notification above raised â€” can a human approve from their phone, not just find out
  ...
  `Notifier::notify_proposal` method, Telegram-only override) with **pure code, no LLM** â€” a tap
  ...
  exactly like a human's Obsidian edit â€” no execution logic duplicated. Revise is the one
  ...
  writes back still `Pending` with fresh buttons â€” only a subsequent Approve tap (pure code) can ever
...
**Hardening pass complete.** All three gaps this pre-Phase-3 pass set out to close â€” the zone-write
guard, resource-budget bounds, and unattended-run visibility (now full round-trip approval, not just
a notification) â€” are done and live-verified. Combined with the already-fixed real bug and two
...
**âœ… Checkpoint #3 fully done (2026-07-04)** â€” both halves, see below.

- **âœ… Event-source trait + cron â€” done (2026-07-04).** The seam Decision 18/19 named: a new
  `EventSource` trait (`liberado-common`) that `Daemon::run` fans into one channel, reacting to
  whatever arrives regardless of source. Built in the explicit sequencing the user chose â€” the
  *existing* vault-watch loop was refactored into the trait's **first** conformer
  ...
  suite passed unchanged, proving the seam is a true no-op) â€” before cron, its second conformer,
  was added. New `liberado-cron` crate (`Schedule`, `CronEventSource`) is deliberately
  vault-agnostic (no `liberado-vault` dependency at all) â€” the concrete proof Decision 19's
  ...
  change both produce reactions over the same channel â€” the literal proof of Decision 18
...
- **âœ… Named dispatcher/executor pools â€” done (2026-07-04).** Checkpoint #3's remaining half:
  ...
  are proven territory â€” a research prompt doc was written, sent to several external research
  ...
  multi-agent research system is strictly orchestrator + narrowed-workers, not peer coordination) â€”
  ...
  unchanged â€” zero call-site breakage anywhere in the codebase). `EventPayload.pool` (set by
  `CronSchedule.pool`/`HookConfig.pool`, `None` â‡’ `"default"`) routes a trigger to its pool;
  `topology.toml`'s new `[[pools]]` declares one, validated fail-fast against schedules/hooks
  naming an undeclared or disabled pool. A pool's authority is nothing new â€” its name **is** the
  ...
  re-verified defensively in `execute_approved`) â€” surfaced to the user as an explicit scope
  ...
  decision referencing the same MCP, one granted it and one not â€” the ungranted pool's dispatcher
  ...
  or share state â€” that's the separate, genuinely open research question, not this slice. Also
  deferred, no concrete need yet: per-pool model/tuning override, per-pool concurrency budgets
  (there's no live concurrency budget to split today â€” `dispatch_parallel`'s semaphore isn't wired
  into the live react() path), zone-based vault-watch pool routing (vault-watch always uses
  `"default"`).
- **âœ… External webhook hook receiver â€” done (2026-07-04).** The other half of "cron and hooks":
  `POST /api/hooks/{name}` (`crates/server/src/hooks.rs`), the push-style counterpart to cron's
  pull-style `EventSource` â€” arbitrary software that can `curl` an HTTP endpoint (systemd
  ...
  producer (the HTTP handler) can inject an `Event` without needing its own `EventSource` loop â€”
  ...
  `secret_ref`, goal â€” parallel to `CronSchedule`), since the stub had no room for a secret and
  nothing else referenced it. Auth is a **per-hook shared secret** (`X-Liberado-Hook-Secret` header,
  constant-time compared) â€” the user's explicit choice over HMAC request signing, for the stated
  goal of being trivially `curl`-able from anything. Idempotency is an in-memory, TTL'd cache keyed
  on an optional `X-Liberado-Idempotency-Key` header â€” the original spec's persisted
  ...
  **Deferred, documented, not silently dropped**: in-process rate limiting (recommendation is a
  reverse proxy if this port is ever exposed beyond a LAN â€” consistent with this project's
  ...
  before any request was sent â€” nothing touched) â€” the automated coverage was judged sufficient
  without repeating it.

### Phase 4 â€” Scaling

- **âœ… Docker MCP transport â€” done (2026-07-07).** The v1 slice of Hermes gap #3 (execution
  ...
  *in the executor crate* would be the wrong layer here: `liberado-executor` is MCP-agnostic and
  ...
  "which environment" really means "where does the MCP *server* process live" â€” a
  ...
  `mcp_registry_from_config` (`crates/bootstrap`) â€” deliberately **no new connector type**:
  ...
  breaks the container's stdin, which a well-behaved MCP server exits on, and `--rm` removes it â€”
  ...
  hibernation (Modal/Daytona-style spin-to-zero) â€” no MCP today has an idle-cost problem that
  justifies the real cloud-backend integration this needs; and wiring `riggers` itself through this
  mechanism for per-task ephemeral sandboxing â€” the generic capability now exists, but `riggers`
  ...
  daemon wasn't running on the dev machine when this landed, so the actual `docker run` â†’ MCP
  handshake path is unit-tested (config round-trip, `docker_argv`, registry registration) but not
  yet proven against a real container â€” see
...
- **Deterministic consequence guard (Â§6 #3)** â€” âœ… *done (per-MCP reversibility/externality)*. Gates
...
- **Zone write-class guard (Â§6 #2)** â€” âœ… *done (2026-07-04, part of the Phase-3 hardening pass â€”
  ...
  was deferred on ("toolâ†’zone resolution") turned out to need new config surface, not just guard
  logic: `McpConfig` gained `default_zone`/`tools: Vec<ToolImpact>` (per-tool zone declarations,
  with unlabeled tools inheriting the MCP's `default_zone` â€” human-authored, like `consequence`,
  ...
  introspection â€” a single generic multi-zone tool (e.g. `vault:write(path)`) needs distinct
...
- **Magnitude / destructiveness axis** â€” âœ… *dispatcher-level done.* A liberado-owned, deterministic
  classifier (`Magnitude`, `is_sweeping_destructive` in `common` â€” reads the goal/tool text, needs no
  MCP metadata) gates **sweeping-destructive** goals even when reversible. Closed the eval's UNSAFE=1:
  "delete all my notes" â†’ `Clarify` (0.90, guard downgrade) while "delete tmp.md" still flows.
  - âœ… **Per-call runtime enforcement â€” done (2026-07-02).** `Orchestrator`'s `ExecuteDirect` /
  ...
  - âœ… **Runtime-gated proposals now land in the vault â€” done (2026-07-02).** That runtime gate's
    downgraded proposals used to write to a data-dir path nothing ever read (a dead end â€” see
    ...
    flows through the exact same approveâ†’execute pipeline pre-flight proposals already use â€” proven
    end-to-end by `daemon`'s
    `runtime_gated_downgrade_lands_in_the_vault_and_executes_once_approved` test.
  - âœ… **Proposal integrity signing â€” done (2026-07-02).** Every `Proposal` now carries an
    ...
    Item 1 (writer-identity verification â€” *who* flipped `status: approved`) stays open; it needs
    OS-level MCP process isolation or an out-of-band approval channel, not a code patch â€” see that
    audit doc's item 1 for why.
- **Proposal workflow (Decision 11)** â€” âœ… *done (emit AND approveâ†’execute landed, June 24, 2026;
  integrity signing + vault-routed runtime proposals added 2026-07-02; `DispatchSubagent` emit
  broadened 2026-07-02).*
  The full proposeâ†’approveâ†’execute loop is closed: a human `status: approved` edit on a
  ...
  `DispatchSubagent` now downgrades to `Propose(ProposedAction::Subagent)` instead of `Clarify` â€”
  ...
  execution a live `DispatchSubagent` gets (unlike the `ToolCalls` arm, which is ungated â€” what was
  ...
  calls) â€” no fixed action to propose there since `ExecuteDirect` carries no goal of its own; see
  `downgrade`'s doc comment in `crates/dispatcher/src/lib.rs` for the shape a follow-up would take.
- **Crate hygiene + hardening passes (2026-07-01 to 2026-07-02)** â€” three hygiene tiers (test-mock
  ...
  items 1, 2, 4, 5 done, item 3 â€” splitting `liberado-common` â€” still deferred).
- âœ… **Dedup/coupling/decomposition/hygiene/coverage audit (2026-07-04)** â€” `cargo dupes` +
  `cargo llvm-cov` + 3 subagents across the whole workspace. Found one real bug (a failed proposal
  write in `RiskGatedToolRuntime` is silently reported to the user as success â€” the proposeâ†’approve
  ...
  closed out 2026-07-07 â€” see below. Full writeup:
  [hygiene-audit-2026-07-04.md](hygiene-audit-2026-07-04.md).
- **Hygiene audit + strategic reprioritization (2026-07-07)** â€” closed out three remaining
  ...
  highest-leverage work â€” the one gap still open against `positioning.md`'s own competitive thesis.
...
- **Shared wire-type + slash-command extraction across clients** â€” TUI, WebUI, and CLI now share
  ...
  done). The plan's proposed `ChatClient` trait was deliberately **not** adopted â€” resolved
...
- **WebUI flesh-out** â€” sidebar, MCP panel, markdown rendering, slash commands, and chat UX landed.
