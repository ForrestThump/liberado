...
| **S6** | **Coding subagents**: parallel worktree children + **parent-only LLM merge-back**. **v1 (landed on develop track):** `payload.subtasks` on a coding goal → N worktrees/branches (`fanout/<label>-i`), bounded concurrency, sequential merge with conflict resolution via merge-role LLM (`crates/coder-agent/src/fanout.rs`, `coder-sandbox` merge helpers). Children never self-merge. **Still open:** hub-spawned child goal sessions (vs in-pack backend fan-out), face `delegate` → coding domain, parent completion gate over merged tree, TUI multi-child chrome. Isolation prerequisite: `WorktreeWorkspace` (S/C7). | Unit: clean dual merge + conflict LLM resolve; live dogfood: two subtasks fan-out. |
