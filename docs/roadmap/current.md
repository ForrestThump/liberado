...
Before starting anything, read [`../architecture/failure-modes.md`](../architecture/failure-modes.md) — six bug classes this codebase produces over and over. Operator knobs (and which are compiled in) are in [`../reference/tuning.md`](../reference/tuning.md). Every one of them shipped with a green test suite.
