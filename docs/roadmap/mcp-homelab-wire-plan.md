...
|---|---|---|---|
| **`liberado-mcp-forge`** | Offline CLI: `cargo install --git` / `--path` → binary under the managed install dir | **Not the runtime path.** Forge explicitly does *not* supervise long-running HTTP servers (see `crates/mcp-forge/ARCHITECTURE.md` non-goals). | Optional: rebuild Liberado-branded stdio binaries for local `McpTransport::Managed` |
...
```
  ┌──────────────── life-os repo ─────────────────┐
  │  mcp-forge  ──sync──►  managed binaries       │  (local/dev; optional)
  │  topology  [[mcps]]  ──►  HttpConnector URLs  │  (homelab: container DNS)
  └───────────────────────────────────────────────┘
                         │
                         ▼  HTTP on `homelab` network
  ┌──────────── already running ──────────────────┐
  │  turbovault(:3001)  weather  caldav  mem0     │
  │  pdf  rentcast  anythingllm  calorie  actual  │
  │  searxng-mcp  spider-mcp  …                   │
  │  images built from github.com/ForrestThump/*  │
  └───────────────────────────────────────────────┘
```
...
|---|---|---|---|---|
| **turbovault** (nginx) → **turbovault-backend** | Custom build from `ForrestThump/turbovault` **`BRANCH=develop`** (Dockerfile default) | `3001` (proxy injects `Origin: http://localhost` for turbomcp allowlist) | `reversible` | `zone_from_arg = "path"` + write_tools list (already in repo template) |
| **liberado-weather-mcp** | `github.com/ForrestThump/liberado-weather-mcp#master` | `8000` | `read_only` | none needed |
| **liberado-pdf-mcp** | `…/liberado-pdf-mcp#master` (FEATURES=native-ocr) | `8080` | `reversible` | `writes_vault = false` |
| **liberado-anythingllm-mcp** | `…/liberado-anythingllm-mcp#master` | `8080` | `reversible` | `writes_vault = false` |
| **liberado-calorie-counter-mcp** | `…/liberado-calorie-counter-mcp#master` | `8080` | `reversible` | `writes_vault = false` (own DB) |
| **caldav-mcp** | `…/liberado-caldav-mcp#master` | `8000` | `reversible` | `writes_vault = false` (calendar server) |
| **rentcast-mcp** | `…/liberado-rentcast-mcp#master` | `8000` | `read_only` | none |
| **mem0-mcp** | `…/liberado-tool-helper-mcp#master` (compose name is historical) | `8000` | `reversible` | `writes_vault = false` (mem0 store) |
...
| **search-orchestrator** | local build | `8080` | TBD / maybe skip | — |
| **deepwiki** | remote SaaS | `https://mcp.deepwiki.com` | `read_only` | none (no container) |

Bare `GET /mcp` often returns 400 — that is normal; streamable-HTTP MCP wants a proper
...
- `url` is **base origin only** — client appends `/mcp` (same footgun as deepwiki).
...
- Zone model (already designed in `config/topology.toml`):
  ...
  ```toml
  zone_from_arg = "path"
  write_tools = ["write_note", "delete_note", "move_note", …]
  ```
...
1. **Local Windows/Linux dev** — `liberado-mcp-forge sync` installs Liberado-branded servers as
   managed stdio binaries so a laptop daemon can use `transport = { kind = "managed" }` without
   Docker Desktop.
2. **Source registry** — `mcp-sources.toml` is the single list of “where does this MCP’s code live,”
...
3. **New MCPs that are not yet containerized** (e.g. `liberado-wakeup-mcp` until it has a compose
   service) — forge → managed binary, or add a GitHub-context build to `docker-compose.ai.yml`.
...
- Replacing Compose’s `build: https://github.com/…#branch` pipeline.
...
GitHub repo (ForrestThump/* or turbovault develop)
        │
        ├─► Homelab Compose  ─build image─► long-running HTTP container
        │                                      ▲
        │                                      │ HttpConnector
        │                                   liberado (topology)
        │
        └─► mcp-sources.toml ─forge sync─► managed binary (dev only)
```
...
Illustrative — wire **one block at a time**, recreate, live-verify, then next.
...
# 1) TurboVault — path-addressed vault ops (develop image via existing compose)
[[mcps]]
name = "turbovault"
...
# 6+) pdf, rentcast, calorie, anythingllm, actual — same HTTP pattern
# Remote:
# [[mcps]]
# name = "deepwiki"
# consequence = "read_only"
...
Matching **policy** (dispatcher grant only — face stays thin via `delegate`):

```toml
[[grants]]
component = "dispatcher"
capabilities = [
    # existing Read/Write zones…
...
## Rollout order (smallest risk → broadest capability)
...
| **10** | **actual / liberado-actual-mcp** | Finance-adjacent — rate carefully (`external`?) | read-only first if possible |
| **11** | **spider-mcp** | Heavy (Chrome); last among local | optional |
| **12** | **deepwiki** | Zero local cost | ask about a public repo |
| later | **liberado-wakeup-mcp** | Add Compose service from GitHub *or* managed binary; register + hook | schedule → webhook fire |
| later | **chat-search** / **memory-mcp** | Workspace-local crates — ship as containers from this repo or managed on host | — |
...
# then a real call: chat "use weather for …" or a goal session that needs the tool
```

If one MCP poisons the shared connect budget, disable it (`enabled = false`) and fix upstream —
template topology already documents this failure mode for weather/rentcast.

---

## mcp-sources.toml (keep as the git map — not the runtime)
...
| Flipping Liberado vault to `:rw` before TurboVault is gated | F1 lesson — capability boundary must be exercised first |
...
4. [ ] searxng → weather → caldav → mem0 → … per table  
...
- Chat/delegate can answer “what’s on my calendar / weather / search X” without OpenClaw  
