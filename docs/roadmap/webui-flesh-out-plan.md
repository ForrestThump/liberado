# WebUI Flesh-Out — Implementation Plan

**Status**: All 5 phases implemented (sidebar, collapsible thinking steps, MCP panel, markdown
rendering, slash commands) — see `crates/webui/src/components/{sidebar,mcp_panel,markdown,slash_commands}.rs`.
...
## Architecture — post-flesh-out layout
...
│              │                                                  │
│  ┌──────────┐ │  ┌────────────────────────────────────────────┐ │
│  │ New Chat │ │  │  Chat History (messages panel)              │ │
│  ├──────────┤ │  │                                            │ │
│  │ Conv 1   │ │  │  ┌─ user bubble                            │ │
│  │ Conv 2   │ │  │  ├─ assistant bubble                       │ │
│  │ Conv 3   │ │  │  │  ├─ thinking step ▸ (collapsed)         │ │
│  │ ...      │ │  │  │  │  └─ 🔧 search-memory ✓ 3 results     │ │
│  ├──────────┤ │  │  │  └─ final answer                        │ │
│  │ MCP       │ │  │  └─ ...                                   │ │
│  │ servers   │ │  └────────────────────────────────────────────┘ │
│  │ (collaps.)│ │  ┌────────────────────────────────────────────┐ │
│  │          │ │  │  Input Bar  [________________] [Send] [⏹] │ │
│  └──────────┘ │  └────────────────────────────────────────────┘ │
└──────────────┴──────────────────────────────────────────────────┘
```
...
**Files touched:**
- `crates/webui/src/main.rs` — restructure layout to sidebar + main
- `crates/webui/src/components/mod.rs` — add `sidebar` module
- `crates/webui/src/components/sidebar.rs` — new: sidebar component with conversation list
- `crates/webui/src/styles/main.css` — sidebar styles, layout changes
...
3. **Chat component changes** (`chat.rs`):
   - Accept `conv_id: Option<String>` prop — when set, loads `GET /api/conversations/{id}` on mount
...
4. **CSS** (`main.css`):
   ...
   - Remove top-level `.chat { height: … }`, replace with `.main-content { display: flex; flex-direction: column; height: 100vh; }` so the chat fills the right pane

5. **Stop button** — add a Stop (⏹) button next to Send in the input bar. During streaming (`sending() == true`), clicking it closes the EventSource and sets `sending(false)`. This wires up the backend stop/cancel primitive already documented in `docs/reference/api.md`.
...
### Phase 2 — Collapsible Thinking Steps
...
**Files touched:**
- `crates/webui/src/components/chat.rs` — enhance `ChatMsg`, new `ThinkingStep` type, new `ThinkingStepView` component
- `crates/webui/src/styles/main.css` — thinking-step drop-down styles
...
┌─ assistant bubble ──────────────────────────────┐
│                                                  │
│  ▸ Thinking (2 steps) — search-memory, read-file │  ← collapsed
│                                                  │
│  Here's what I found in your notes...            │  ← assistant answer text
└──────────────────────────────────────────────────┘

▼ Thinking (2 steps)                              ← expanded
  ┌─ 🔧 search-memory ("hello") ──────────────────┐
  │  ✓ Found 3 results in vault                   │
  └────────────────────────────────────────────────┘
  ┌─ 🔧 read-file (notes.md) ─────────────────────┐
  │  ✓ Read 1,234 bytes                           │
  └────────────────────────────────────────────────┘
...
- Clicking the "▸ Thinking" header toggles between `▸` (collapsed) and `▼` (expanded)
...
**CSS:**
- `.thinking-group` — container with subtle border, rounded, slightly inset from bubble
- `.thinking-header` — clickable row with arrow indicator, tool count, tool name summary
- `.thinking-step` — individual tool call row (compact monospace)
- `.thinking-step.pending` — with spinner or pulse animation
- `.thinking-step .tool-name` — yellow/accent color
- `.thinking-step .tool-result` — green (ok) or red (err)

---

### Phase 3 — MCP Connections Panel
...
**Files touched:**
- `crates/webui/src/components/mcp_panel.rs` — new component
- `crates/webui/src/components/sidebar.rs` — integrate MCP panel as a section
- `crates/webui/src/styles/main.css` — MCP panel styles
...
┌─ MCP Servers ──────────────────────── ▼ ─┐
│                                           │
│  ▸ vault (reversible, 3 tools)            │
│    ├─ read                                │
│    ├─ write                               │
│    └─ search                              │
│                                           │
│  ▸ tasks-mcp (reversible, 2 tools)        │
│    ├─ add                                 │
│    └─ list                                │
│                                           │
│  ▸ deepwiki (read_only, 1 tool)           │
│    └─ ask_question                        │
└───────────────────────────────────────────┘
```
...
- `read_only` → `--lib-reaction-observed` (cyan)
- `reversible` → `--lib-reaction-acted` (green)
- `irreversible` / `external` → `--lib-tool-err` (red)

**CSS additions:**
- `.mcp-panel` — matches card style
- `.mcp-server` — row with name + badge + tool count
- `.mcp-server-header` — clickable, expand/collapse
- `.mcp-tool-list` — indented monospace list
- `.mcp-tool` — individual tool name
- `.consequence-badge` — pill-shaped colored indicator

---

### Phase 4 — Markdown Rendering (stretch)

**Goal:** Render assistant/user messages as HTML from Markdown using `pulldown-cmark`.

**Files touched:**
- `crates/webui/Cargo.toml` — add `pulldown-cmark`
- `crates/webui/src/components/markdown.rs` — new: markdown-to-Dioxus-Element renderer
- `crates/webui/src/components/chat.rs` — use markdown renderer for assistant/user bubbles
- `crates/webui/src/styles/main.css` — markdown element styles (code blocks, lists, links, etc.)

### Phase 5 — Slash Commands (stretch)

**Goal:** Wire the scaffolded `slash_commands.rs` into the chat input.

**Files touched:**
- `crates/webui/Cargo.toml` — move `liberado-commands` into wasm32 deps
- `crates/webui/src/components/mod.rs` — add `slash_commands` module
- `crates/webui/src/components/chat.rs` — integrate slash command handling in submit
- `crates/webui/src/components/slash_commands.rs` — make it compile and work

---

## Order of Implementation

1. **Phase 1 first** — it restructures the layout, which everything else builds on
2. **Phase 2 second** — the thinking steps are the most visible new feature
3. **Phase 3 third** — MCP panel slots into the sidebar built in Phase 1
4. **Phase 4 optionally** — markdown is nice-to-have, not core to the UI structure
5. **Phase 5 optionally** — slash commands are already scaffolded

## Verification

After each phase:
```powershell
# Start the daemon (API only — no need to rebuild the wasm bundle for hot-reload dev)
