...
| [current.md](current.md) **CH4** | Mid-session / per-conversation model switching (open; process-wide hot-swap already exists) |
...
| [session-profiles-plan.md](session-profiles-plan.md) | Per-conversation tool authority (steps 1–3 landed on `feat/session-profiles`) |
...
Start every planning session at [current.md](current.md).
