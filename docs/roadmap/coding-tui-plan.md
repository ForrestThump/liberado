...
**Umbrella architecture**: [`agentic-loops.md`](../architecture/agentic-loops.md) (kernel vs pack),
...
[`loops-plan.md`](loops-plan.md) (`/loop` machinery), [`session-surface-contract.md`](../architecture/session-surface-contract.md)
(what any session surface owes the user), [`verifiers.md`](../architecture/verifiers.md) (intake +
frozen criteria), [`failure-modes.md`](../architecture/failure-modes.md) (read before building).
...
`/api/loops*`, park/resume/rewind/diff endpoints), `current.md` (slice tracking).
