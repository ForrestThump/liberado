...
| [`liberado-conversation-store`](../../crates/conversation-store/) | `liberado-provider` | The Decision-17 conversation contract: the ConversationStore trait plus the message-node DAG types (id, parent_id, Author) every session transcript is made of. Production storage now lives in liberado-session-store, which implements this trait; the JsonlStore here is the original single-purpose implementation. |
