...
40 workspace crates as of 2026-07-11.
...
| [`liberado-session`](../../crates/session/) | `liberado-common` | Domain-neutral goal session kernel: GoalSpec, SessionEvent envelope, in-memory hub, DomainPackRunner trait. Surfaces (TUI/WebUI) are clients of this contract — not loop owners. |
...
| Crate | Internal deps | Description |
|---|---|---|
| [`liberado-chat-search`](../../crates/chat-search/) | `liberado-conversation-store`, `liberado-provider` | Full-text and regex search over Liberado conversation history (the JSONL conversation store). Query parsing, per-file scanning, snippet extraction — shared by liberado-server's REST endpoint and the chat-search MCP tool so search logic exists exactly once. |
| [`liberado-conversation-store`](../../crates/conversation-store/) | `liberado-provider` | The Decision-17 conversation-history store: an append-only log of message nodes (a DAG; linear chat is the degenerate case), persisted as JSONL outside the vault with time-sortable ids minted at append. The v1 JsonlStore behind the ConversationStore trait. |
...
| Crate | Internal deps | Description |
|---|---|---|
| [`liberado-coder-agent`](../../crates/coder-agent/) | *none* | Coding domain pack: Liberado goal-session backend over Executor + coding tools (not the mesh kernel) |
...
| [`liberado-daemon`](../../crates/daemon/) | `liberado-common`, `liberado-notify`, `liberado-vault`, `liberado-dispatcher`, `liberado-orchestrator` | The Liberado daemon (Decision 2, daemon-first): the long-running core that watches the vault, attributes changes (loop-breaking), and emits reactable events. v1 vertical slice. |
| [`liberado-server`](../../crates/server/) | `chat-client-contract`, `liberado-bootstrap`, `liberado-chat-search`, `liberado-common`, `liberado-config`, `liberado-daemon`, `liberado-dispatcher`, `liberado-orchestrator`, `liberado-mcp`, `liberado-executor`, `liberado-main-agent`, `liberado-conversation-store`, `liberado-provider`, `liberado-telegram-approvals`, `liberado-memory-store`, `liberado-vault`, `liberado-session`, `liberado-coder-agent` | The Liberado daemon's API server (library): the watch loop + chat + HTTP/SSE API. Runnable via `liberado serve`. |
