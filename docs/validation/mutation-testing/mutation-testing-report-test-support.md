...
`1a452e09ff15091ed19fea8305bd7f7770e078bf` contains the added regression tests.
...
reviewed run used the default `--timeout 3.0 --minimum-test-timeout 30` and `--in-place`. It
tested 117 mutants in 5m after a 6s baseline build and 2s baseline test.
...
| `crates/test-support/src/mvl_oracle.rs:106` `failed` → `vec![]` | Empty return, loses Fail verdicts | `failed_returns_only_fail_verdicts` |
| `crates/test-support/src/mvl_oracle.rs:108` `status == Fail` → `!= Fail` | Inverted filter | `failed_returns_only_fail_verdicts` |
| `crates/test-support/src/mvl_oracle.rs:113` `all_checked_passed` → `true` | Always true regardless of verdicts | `all_checked_passed_*` (same pair) |
...
| `crates/test-support/src/mvl_oracle.rs:349` `oracle_usage` → `""` / `"xyzzy"` | The CLI binary (`mvl-conformance`) prints usage with the static string; no test asserts the exact text content of the usage message. A test that asserts `oracle_usage().contains("--mvl")` survives both mutations. | No test reads the literal value. |
| `crates/test-support/src/trace_contracts.rs:357` delete match arm `"prompt"` in `assert_join_integrity` | Full prompts stop contributing to `mvl_full_prompt_turns`, so a context transform can no longer join through a later full prompt. Existing fixtures either have `context_changed` or test other prompt rules. | Needs a transform fixture that joins only through a later full prompt. |
...
remaining misses are: 2 structural equivalents (`Vec::new()` identity), 2 untested usage-string
replacements, 1 deleted prompt arm, 3 multi-run or single-difference fixture gaps, and 1
`reconstruct_all_turns` mutation. The added tests cover the quick-win surface (`MockNotifier`, `ConformanceReport`, `kill_prefix`,
