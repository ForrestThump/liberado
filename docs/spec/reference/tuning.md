...
| `chat` | `agent_profiles` | named session profiles that classify a conversation as `SurfaceMode::Agent` on the chat-surface shelf (CAS1). Default set: `coding`, `life`, `researcher`, `operator` — deployments extend or shrink. `chat-default` is the Chat-shelf hat (CAS3) and is **not** in this set. The same set drives the `ChatSessions` create-time stamp, the `SessionStore` chat-lens projection on every read, the `GET /api/profiles` `agent_eligible` field (which the WebUI New Agent picker uses), and the privileged face `create_agent` tool. History search (`chat-search`) is a commented opt-in on the `main-agent` grant in `config.example/policy.toml`, not a tuning knob. See [`chat-agent-surface-mode.md`](../architecture/chat-agent-surface-mode.md) §6c / §6d. |
