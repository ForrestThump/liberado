...
| `chat` | `agent_profiles` | named session profiles that classify a conversation as `SurfaceMode::Agent` on the chat-surface shelf (CAS1). Default set: `coding`, `life`, `researcher`, `operator` — deployments extend or shrink |
