...
| [`liberado-cli`](../../../crates/cli/) | `liberado-common`, `liberado-server`, `liberado-coder-core`, `chat-client-contract` | the `liberado` binary: a client + launcher — `serve` runs the daemon/API, `chat` is a streaming client |
