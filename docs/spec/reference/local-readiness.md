...
`Config::validate_schedules` on `crates/config-loader/src/model/config.rs`: the four
per-job-kind `if`/`&&` checks (job-kind allowlist, `habit-ping` requires `habit_text`,
...
past 10 branches). `build_event` on `crates/cron/src/lib.rs`: the new
...
sibling method, so the inline shape matches the function's existing pattern. All three ceilings
sit at 21 (above the new-function ceiling of 20, per the same rule as the cross-provider
fallback waivers). Each waiver carries a hard ceiling and review date; any further growth in
these functions is not covered and must be split, not waived.
