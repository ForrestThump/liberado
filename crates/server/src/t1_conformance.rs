...
//!
//! Landed checks: **L2** (durable reopen), **L5** (AskHuman 403), **L6** (Write both-arms),
//! **L8** (cancel → Cancelled), **L10** (fork prefix snapshot).
...
struct T1Harness {
    ...
    /// Keep the temp dir alive for the test lifetime.
    _root: PathBuf,
}
