...
impl ContinuationContextBuilder {
    ...
    pub fn build(record: &TaskRecord) -> String {
        ...
        append_header(&mut out, record);
        append_worktree(&mut out, record);
        append_failures(&mut out, record);
        append_instructions(&mut out);
        out
    }
}

fn append_header(out: &mut String, record: &TaskRecord) {
    out.push_str(&format!(
...
fn append_worktree(out: &mut String, record: &TaskRecord) {
...
fn append_failures(out: &mut String, record: &TaskRecord) {
...
fn append_instructions(out: &mut String) {
