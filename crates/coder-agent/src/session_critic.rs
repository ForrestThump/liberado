...
fn instructions(prompt_dir: Option<&std::path::Path>) -> String {
...
pub async fn review_session(
    ...
    let mut completion = CompletionRequest::new(vec![
        Message::system(instructions(Some(&liberado_coder_core::prompts::dir_for(
            request.config.prompt_dir.as_deref(),
