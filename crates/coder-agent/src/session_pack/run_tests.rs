...
// The four fan-out scenarios below drive real `git worktree add`/`remove`
// against temp repos. On the Windows CI runner they fail inside git itself
// with two distinct, environment-shaped errors (`not a git repository:
// ...RUNNER~1...` and `fatal: this operation must be run in a work tree`)
// even with children serialised; both pass on developer Windows boxes and
// always on Linux. The runner resolves tempdirs through 8.3 short paths,
// which git's worktree backlink bookkeeping does not survive. The same
// production paths stay covered on Windows through
// session_pack::build_survivor_tests, whose fixtures avoid linked
// worktrees. Revisit if the runner condition changes.
