...
mod tests {
    ...
    // ── Log-capture harness (same pattern as orchestrator's build-line test) ──
    //
    // The `log_*` helpers and `normalize_mcp_list`'s normalization note are observable only
    // through emitted events, so their tests install a capturing subscriber. A mutant that
    // deletes one of those bodies changes nothing else — without capture it survives.
    ...
    /// Every `DispatchAction` variant must reach the pre-guard log line — the dogfood trace
    /// reads these to reconstruct what the classifier did, so a silently dropped arm (or a
    /// deleted helper body) breaks observability, not just verbosity.
    #[test]
    ...
    /// The normalization debug note fires when an entry needed rewriting (`mcp:tool` → catalog
    /// name), not when it was already canonical — and unknown MCPs are dropped with a warning.
    #[test]
