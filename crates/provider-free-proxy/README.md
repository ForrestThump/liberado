...
On 429 / rate-limit / 5xx / timeout / candidate-scoped transport error / quota
exhaustion, the proxy falls through to the next-best remaining free model, up
to `LIBERADO_FREE_PROXY_MAX_ATTEMPTS` (default 6). Each attempt uses **that
model's** provider base and key. A 400 payload error does not spend another
candidate.
...
| `CEREBRAS_API_KEY` | `https://api.cerebras.ai/v1` | rate-limited free |
