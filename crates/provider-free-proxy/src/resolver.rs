...
//!   serves the previous snapshot rather than erroring — an old ordering beats no ordering at all,
//!   and only a first-ever discovery failure is fatal.
//!
...
impl BestFreeModelResolver {
    ...
    ///
    /// Errors only when there has never been a successful discovery.
    ...
    async fn refresh_locked(&self) -> Result<Resolution, ResolveError> {
        match self.resolve_once().await {
            ...
            Err(e) => {
                // Degraded-refresh path: serve yesterday's ordering rather than failing calls
                // because a benchmark endpoint hiccupped.
    ...
    async fn resolve_once(&self) -> Result<Resolution, ResolveError> {
        ...
        let api_ok = match self.benchmarks.coding_benchmark_rows().await {
            Ok(rows) => {
        ...
        let origin = match (api_ok, scraped_matched > 0) {
            (true, _) => Origin::BenchmarksApi,
            (false, true) => Origin::Scraped,
            (false, false) => Origin::Heuristic,
        };
