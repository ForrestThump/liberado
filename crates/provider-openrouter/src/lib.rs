...
//! evaluations at once (the planned heuristics tuning engine,
//! `docs/roadmap/heuristics-tuning-engine-plan.md`) without getting rate-limited by a single
//! upstream provider's per-key limits.
//!
//! Like [`liberado_provider_deepseek`], this is a thin translation layer: map the normalized
//! [`CompletionRequest`] to the OpenAI wire shape, POST it, map the response back. The translation
//! is split into pure functions ([`to_openai_request`], [`from_openai_response`]) so the mapping is
//! unit-tested deterministically without a network call or an API key; a single `#[ignore]`d smoke
//! test exercises the real endpoint.
//!
//! **Known duplication, deliberately not yet factored out**: this crate's translation logic
//! (tool-name sanitization, request/response mapping, SSE stream assembly) is near-identical to
//! `liberado-provider-deepseek`'s, because both backends speak the same OpenAI-compatible wire
//! format. Lifting the shared parts into a common crate is real work with its own design questions
//! (what's genuinely shared vs. provider-specific quirks) and wasn't the point of this pass — the
//! point was getting a working, testable OpenRouter backend with minimal blast radius on the
//! existing, working DeepSeek provider. Worth doing once a third OpenAI-compatible backend shows
//! up (the pattern proves out) or before the heuristics tuner ships for real (see
//! `crate-modularity-audit.md`'s finding 3 for the shape this kind of deferred-dedup call usually
//! takes in this repo).
//!
...
pub(crate) fn to_openai_request(
    ...
    if matches!(req.response_format, ResponseFormat::Json { .. }) {
        // Most OpenRouter-routed models that support JSON mode honor the same "type":
        // "json_object" hint as OpenAI; models that don't support it ignore or reject it —
        // picking a capable model is the caller's job (see the module doc comment).
...
fn parse_tool_call(tc: &Value, name_map: &ToolNameMap) -> Option<ToolInvocation> {
    ...
    // OpenAI (and OpenRouter) encode arguments as a JSON *string*; parse it back to a value
    // (empty on failure).
...
mod tests {
    ...
    fn request_maps_messages_temperature_and_json_mode() {
        ...
        let body = to_openai_request("openai/gpt-4o-mini", &req, &empty_name_map());
    ...
    fn request_maps_tools_and_tool_messages() {
        ...
        let body = to_openai_request("openai/gpt-4o-mini", &req, &name_map);
