...
/// The executor-layer analog of [`cold_start`] — same Monte Carlo restart shape, describing an
/// executor's job (bounded tool-use loop, finish via `submit_report`) instead of a dispatcher's.
...
/// The executor-layer analog of [`mutate`] — same targeted-fix shape, but the failure context comes
/// from [`ToolLoopScoredScenario`] (which tools were/weren't called, what outcome resulted) instead
/// of a dispatcher misroute.
