//! The dispatcher-layer meta-LLM calls that produce candidate system prompts: [`cold_start`] (a
//! fresh, independent restart) and [`mutate`] (a targeted fix for a parent's known failures), plus
//! [`request_justification`] (shared with the executor/subagent layer too — see
//! [`crate::tool_loop_generation`] for that layer's `cold_start_executor`/`mutate_executor`
//! analogs). All go through `liberado_provider::complete_json` — the same structured-output helper
