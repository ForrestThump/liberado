...
mod external;
mod runner_support;
use external::{DeepAgentsAdapter, HermesAdapter, append_external_harness_pins};
...
pub(crate) fn run_args_from_spec(
    ...
    apply_harness_overrides(&mut args, &spec.harnesses);
    args
}

fn apply_harness_overrides(args: &mut RunArgs, harnesses: &[crate::contract::HarnessRequest]) {
    for harness in harnesses {
        apply_binary_override(args, harness);
        apply_git_sha_override(args, harness);
    }
}

fn apply_binary_override(args: &mut RunArgs, harness: &crate::contract::HarnessRequest) {
    let Some(binary) = &harness.binary else {
        return;
    };
...
fn apply_git_sha_override(args: &mut RunArgs, harness: &crate::contract::HarnessRequest) {
    let Some(sha) = &harness.git_sha else {
        return;
    };
    match harness.id.as_str() {
...
fn write_run_pins(
    ...
    append_external_harness_pins(&mut pins, args);
