//! The Liberado topology adapter: reads `topology.toml` and turns the declared runtime instances
//! (providers, MCP servers, pools, profiles, projects, schedules, hooks, the vault, the notifier)
//! into [`MapNode`]s with the metadata the profile's rules key on. This is the one genuinely
//! Liberado-specific piece of the map — the cargo scanner and assembly live in `sysmap-core`.
...
/// An error while reading the runtime topology.
#[derive(Debug)]
pub enum ScanError {
    Io {
        path: PathBuf,
