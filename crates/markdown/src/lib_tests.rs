    ...
    fn bold_then_tail_exact() {
        ...
        assert_eq!(
            spans.iter().map(|s| (s.text.as_str(), s.style)).collect::<Vec<_>>(),
    ...
    fn code_span_boundaries_exact() {
        ...
        assert_eq!(
            leading.iter().map(|s| (s.text.as_str(), s.style)).collect::<Vec<_>>(),
            vec![("c", SpanStyle::CODE), (" x", SpanStyle::NONE)]
        );
        let trailing = paragraph_spans("x `c`");
        assert_eq!(
            trailing.iter().map(|s| (s.text.as_str(), s.style)).collect::<Vec<_>>(),
