...
#   2. Optionally ships leftover nested `turbovault/` and `turbomcp/` clones if they exist
#      locally. They are gitignored and are not required: Cargo fetches the ForrestThump git+tag
#      pins. A leftover clone is copied as a working tree only.
...
#
# Note: <ref> selects the MAIN repo commit only. Leftover turbovault/turbomcp clones are shipped
# from on-disk working copies when present; Cargo fetches the git+tag pins without them.
#
...
REF="${1:-HEAD}"
VENDORED="turbovault turbomcp"                    # optional leftover clones; git+tag pins do not need them
...
# Leftover nested clones are optional. Cargo fetches the git+tag pins without them.
existing_vendored=""
for v in $VENDORED; do
  if [ -d "$v" ]; then
    existing_vendored="$existing_vendored $v"
  fi
done

say "Deploying main @ $SHORT  ($DESC)"
if [ -n "$existing_vendored" ]; then
  say "  + leftover clones:$existing_vendored"
else
  say "  + no leftover nested clones (Cargo git+tag pins)"
fi
...
if [ -n "$existing_vendored" ]; then
  ...
  tar -cf - --exclude='.git' --exclude='target' --exclude='node_modules' $existing_vendored \
    | ssh "$SSH_TARGET" "tar x -C ~/$STAGE"
fi
